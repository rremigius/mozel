...
import Property, { Alphanumeric, isComplexValue, isModelClass } from './Property';
...
import { ModelFactoryType } from "@/ModelFactoryInterface";
...
let Model = Model_1 = class Model {
    constructor(modelFactory, registry) {
        this.properties = {};
        this.parent = null;
        this.parentLock = false;
        this.relation = null;
        this.gid = 0; // a non-database ID that can be used to reference other models
    ...
    watch(watcher) {
        ...
        if (watcher.immediate) {
            Model_1.callWatcherHandler(watcher, get(this, watcher.path));
        }
        else {
    ...
    propertyChanged(path, newValue, oldValue) {
        ...
        this.watchers.forEach(watcher => {
            ...
            if (pathStr === watcher.path) {
                Model_1.callWatcherHandler(watcher, newValue);
                return;
            }
            ...
            if (watcher.path.substring(0, pathStr.length) === pathStr) {
                ...
                if (newWatcherValue !== watcher.currentValue) {
                    Model_1.callWatcherHandler(watcher, newWatcherValue);
            ...
            if (watcher.deep && pathStr.substring(0, watcher.path.length) === watcher.path) {
                Model_1.callWatcherHandler(watcher, get(this, watcher.path)); // cannot keep track of previous value without cloning
            }
        });
    }
    /**
...
Model._classPropertyDefinitions = [];
Model._classCollectionDefinitions = [];
__decorate([
    property(Alphanumeric)
], Model.prototype, "id", void 0);
__decorate([
    property(Alphanumeric, { required })
], Model.prototype, "gid", void 0);
Model = Model_1 = __decorate([
    injectable(),
    __param(0, inject(ModelFactoryType)), __param(0, optional()),
