...
export class MozelWatcherEvents extends EventInterface {
    constructor() {
        super(...arguments);
        this.changed = this.$event(MozelWatcherChangedEvent);
    }
}
export class MozelWatcher {
    ...
    constructor(mozel, options) {
        this.watchers = [];
        this._changes = {};
        this.newMozels = new Set();
        this.mozelsInUpdates = new Set();
        this.stopCallbacks = [];
        this.version = 0;
        this.history = [];
        this.events = new MozelWatcherEvents();
    ...
    start(includeCurrentState = false) {
        ...
        this.stopCallbacks.push(this.mozel.$registry.events.added.on(event => {
            const mozel = event.item;
            if (!(mozel instanceof Mozel) || this.mozelsInUpdates.has(mozel.gid))
                return;
            /*
