...
export default class Collection<T extends Mozel|primitive> extends Mozel {
   ...
   */
	...
	set(index:number, value:object|T, init = true, merge = false, fireEvents = true) {
		const current = this._$list[index];
		...
		if(this.isReference && index > this._$list.length) {
			return false;
		}
		this._$list[index] = revised;
	...
	$setData(items:Array<object|T>, init = true, merge = false) {
		...
		const before = this._$list.slice();
		...
		for(let i = this._$list.length-1; i >= items.length; i--) {
			this._$list.splice(i, 1);
		}


		const mutations = Collection.getMutations(before, this._$list);
